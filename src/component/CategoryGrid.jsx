    ...
    <div className="mx-28 py-10">
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-6">
        {categories.map((cat, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between bg-white rounded-full shadow-sm border border-gray-100 p-2 pr-4 hover:shadow-md transition cursor-pointer group"
          >
            ...
            {/* Right side - Arrow */}
            <div className="w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center group-hover:bg-pink-200 transition">
              <ChevronRight className="text-pink-500 w-3 h-3" />
            </div>
          </div>
        ))}
      </div>
